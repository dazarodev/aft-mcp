...
**M001 (Foundation), M002 (Language Intelligence), M003 (Call Graph Navigation) complete.** The `aft` Rust binary runs as a persistent process with NDJSON protocol, embeds tree-sitter grammars for 6 languages (TypeScript, JavaScript, TSX, Python, Rust, Go), and handles 26 domain commands: the original 11 from M001, 3 import commands, 5 structure commands, `transaction`, and 6 call graph commands (`configure`, `call_tree`, `callers`, `trace_to`, `impact`, `trace_data`). All 12 mutation commands support `dry_run: true` and auto-format. The call graph engine builds lazily from a configured project root, resolves cross-file calls through import chains, supports forward call trees, reverse caller lookup, backward trace-to-entry-point traversal, change impact analysis with signature/entry-point annotations, and data flow tracking through assignments and cross-file parameter matching. Entry point detection uses generic heuristics (exported functions, main/init patterns, test patterns per language). A `notify` v8 file watcher invalidates stale graph data on file changes. 368 Rust tests + 39 plugin tests pass. Next: M004 (Refactoring Primitives).
