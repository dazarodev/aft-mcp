...
fn write_dry_run_returns_diff() {
    ...
    assert!(
        diff.contains("--- a/"),
        "diff should have a/ prefix: {}",
        diff
    );
    assert!(
        diff.contains("+++ b/"),
        "diff should have b/ prefix: {}",
        diff
    );
    assert!(
        diff.contains("-const x = 1;"),
        "diff should show removed line: {}",
        diff
    );
    assert!(
        diff.contains("+const x = 2;"),
...
"#;
    ...
    assert!(
        diff.contains("-    return `Hello,"),
        "diff should show removed line: {}",
        diff
    );
    assert!(
        diff.contains("+    return `Hi,"),
        "diff should show added line: {}",
        diff
    );
    ...
    assert_eq!(
        on_disk, original,
        "file MUST NOT be modified by dry-run edit_symbol"
    );
...
fn edit_match_dry_run() {
    ...
    let diff = resp["diff"].as_str().expect("diff should be a string");
    assert!(
        diff.contains("-const greeting = \"hello\";"),
        "diff should show removed line: {}",
        diff
    );
    assert!(
        diff.contains("+const greeting = \"world\";"),
        "diff should show added line: {}",
        diff
    );
...
fn batch_dry_run() {
    ...
    assert!(
        diff.contains("-const a = 1;"),
        "diff should show first edit: {}",
        diff
    );
    assert!(
        diff.contains("+const a = 10;"),
        "diff should show first replacement: {}",
        diff
    );
    assert!(
        diff.contains("-const c = 3;"),
        "diff should show second edit: {}",
        diff
    );
    assert!(
        diff.contains("+const c = 30;"),
        "diff should show second replacement: {}",
        diff
    );
...
fn add_import_dry_run() {
    ...
    let diff = resp["diff"].as_str().expect("diff should be a string");
    assert!(
        diff.contains("useEffect"),
        "diff should mention useEffect: {}",
        diff
    );
...
fn dry_run_no_backup() {
    ...
    assert!(
        entries.is_empty(),
        "dry-run should not create backup entries, got: {:?}",
        entries
    );
...
fn dry_run_syntax_validation() {
    ...
    assert_eq!(
        ...
        "broken syntax should report syntax_valid:false: {:?}",
        resp
    );
...
fn dry_run_empty_diff() {
    ...
    assert!(
        ...
        "no-op should produce empty or trivial diff: {}",
        diff
    );
