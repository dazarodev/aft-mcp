...
fn test_checkpoint_create_restore_cycle() {
    ...
    // Restore checkpoint
    let resp = aft.send(r#"{"id":"cp-restore","command":"restore_checkpoint","name":"safe-point"}"#);
...
fn test_undo_no_history_error() {
    ...
    assert!(resp["message"].as_str().unwrap().contains(&file.display().to_string())
        .then_some(true)
        .or_else(|| Some(resp["message"].as_str().unwrap().contains("no undo history")))
        .unwrap());
