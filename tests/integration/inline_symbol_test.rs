...
fn inline_symbol_expression_body() {
    ...
    assert_eq!(resp["ok"], true, "inline expression body should succeed: {:?}", resp);
...
fn inline_symbol_scope_conflict() {
    ...
    assert_eq!(resp["ok"], false, "should fail with scope_conflict: {:?}", resp);
    assert_eq!(resp["code"], "scope_conflict");

    let conflicting = resp["conflicting_names"].as_array().expect("conflicting_names array");
    assert!(
    ...
    for s in suggestions {
        assert!(s["original"].as_str().is_some(), "suggestion should have 'original': {:?}", s);
        assert!(s["suggested"].as_str().is_some(), "suggestion should have 'suggested': {:?}", s);
    }
