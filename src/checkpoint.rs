...
impl CheckpointStore {
    ...
    pub fn restore(&self, name: &str) -> Result<CheckpointInfo, AftError> {
        let checkpoint = self.checkpoints.get(name).ok_or_else(|| {
            AftError::CheckpointNotFound {
                name: name.to_string(),
            }
        })?;
...
mod tests {
    ...
    fn overwrite_existing_name() {
        ...
        store.create("dup", vec![path.clone()], &backup_store).unwrap();
    ...
    fn list_returns_metadata() {
        ...
        store.create("a", vec![path.clone()], &backup_store).unwrap();
        store.create("b", vec![path.clone()], &backup_store).unwrap();
    ...
    fn cleanup_removes_expired() {
        ...
        store.create("recent", vec![path.clone()], &backup_store).unwrap();
