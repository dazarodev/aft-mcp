...
fn is_keyword(name: &str, lang: LangId) -> bool {
    match lang {
        ...
        LangId::Python => matches!(
            name,
            "None" | "True" | "False" | "print" | "len" | "range" | "str" | "int" | "float"
                | "list" | "dict" | "set" | "tuple" | "type" | "super" | "isinstance"
                | "enumerate" | "zip" | "map" | "filter" | "sorted" | "reversed" | "any"
                | "all" | "min" | "max" | "sum" | "abs" | "open" | "input" | "format"
                | "hasattr" | "getattr" | "setattr" | "delattr" | "repr" | "iter" | "next"
...
pub fn detect_return_value(
    ...
    collect_declarations_in_range(&root, source, start_byte, end_byte, lang, &mut in_range_decls);
    ...
    if let Some(fn_end) = enclosing_fn_end_byte {
        ...
        if end_byte < post_range_end {
            ...
            collect_identifier_refs(&root, source, end_byte, post_range_end, lang, &mut post_refs);
...
pub fn generate_extracted_function(
    ...
    match lang {
        LangId::TypeScript | LangId::Tsx | LangId::JavaScript => {
            generate_ts_function(name, params, return_kind, body_text, base_indent, indent_unit)
        }
        LangId::Python => {
            generate_py_function(name, params, return_kind, body_text, base_indent, indent_unit)
        }
        _ => {
...
fn generate_ts_function(
    ...
    lines.push(format!("{}function {}({}) {{", base_indent, name, params_str));
...
mod tests {
    ...
    fn free_vars_detects_enclosing_function_params() {
        ...
        let result = detect_free_variables(&source, &tree, line5_start, line6_end, LangId::TypeScript);
        assert!(
...
"#;
    ...
    fn return_value_post_range_usage() {
        ...
        let result = detect_return_value(&source, &tree, start, end, Some(fn_end), LangId::TypeScript);
    ...
    fn return_value_void() {
        ...
        let result = detect_return_value(&source, &tree, start, end, Some(crate::edit::line_col_to_byte(&source, 23, 0)), LangId::TypeScript);
    ...
    fn scope_conflicts_none() {
        ...
        let conflicts = detect_scope_conflicts(source, &tree, call_byte, &[], body_text, LangId::TypeScript);
    ...
    fn scope_conflicts_detected() {
        ...
        assert!(conflicts.iter().any(|c| c.name == "temp"), "conflicts: {:?}", conflicts);
        assert!(conflicts.iter().any(|c| c.suggested == "temp_inlined"), "should suggest temp_inlined");
    }
    ...
    fn substitute_params_whole_word() {
        ...
        assert!(!result.contains("items") || result.contains("items"), "items should be preserved");
        // The `i` in `i => i > 0` should be replaced
        assert!(result.contains("index"), "should contain 'index': {}", result);
    }
