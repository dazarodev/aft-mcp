...
pub fn handle_inline_symbol(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    let sym = match matches.iter().find(|m| {
        ...
        None => {
            return Response::error(
                ...
                "symbol_not_found",
                format!(
                    "inline_symbol: no function '{}' found in {}",
                    symbol, file
                ),
            );
        }
    ...
    let replacement_text =
        build_inline_replacement(&substituted_body, &call_context, &replacement_indent, lang, assignment_var.as_deref());
    ...
    let backup_id =
        match edit::auto_backup(ctx, path, &format!("inline_symbol: {}", symbol)) {
            Ok(id) => id,
            Err(e) => {
                return Response::error(&req.id, e.code(), e.to_string());
            }
...
mod tests {
    ...
    fn inline_symbol_scope_conflict_reported() {
        ...
        let resp = handle_inline_symbol(&req, &ctx);
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["ok"], false, "should fail with scope_conflict: {:?}", json);
