...
pub fn handle_extract_function(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    // --- Detect return value ---
    let return_kind =
        detect_return_value(&source, &tree, start_byte, end_byte, enclosing_fn_end_byte, lang);
    ...
    let call_site = generate_call_site(name, &free_vars.parameters, &return_kind, &range_indent, lang);
    ...
    let new_source = build_new_source(&source, insert_pos, start_byte, end_byte, &extracted_fn, &call_site);
    ...
    eprintln!(
        ...
        name, file, start_line, end_line, param_count
    );
...
mod tests {
    ...
    fn extract_function_missing_file() {
        ...
        assert!(msg.contains("file"), "message should mention 'file': {}", msg);
    }
